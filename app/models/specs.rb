...
class Specs
	...
	def self.get_specs
		...
		rows = ActiveRecord::Base.connection.execute("SELECT specs.id AS id, classes.name AS class, specs.name AS spec, role, description, background_image, icon FROM specs JOIN classes ON specs.class_id=classes.id ORDER BY spec ASC")
    rows.each do |row|
    	clazz = row["class"]
      spec = Spec.new(row["id"], clazz, row["spec"], row["role"], row["description"], row["background_image"], row["icon"])
