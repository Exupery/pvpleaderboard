class LeaderboardsController < BracketRegionController
  ...
  def last_ranking
    ...
    rows = get_rows("SELECT MAX(ranking) AS ranking FROM leaderboards WHERE bracket='#{@bracket}' #{@region_clause}")
  ...
  def total
    ...
    rows = get_rows("SELECT COUNT(*) AS count FROM leaderboards WHERE bracket='#{@bracket}' #{@region_clause}")
